...
import net.turtleboi.turtlerpgclasses.rpg.talents.warriorTalents.CombatVeteranTalent;

import java.util.ArrayList;
import java.util.List;

public class CombatVeteranTalentNode extends StatTalentButton {
    public CombatVeteranTalentNode(TalentTree talentTree, Talent talentClass, int x, int y, int maxPoints, int requiredPoints, boolean alwaysActive, OnPress onPress) {
    ...
    public List<Component> generateDynamicTooltip() {
        if (talentClass instanceof CombatVeteranTalent talent) {
            List<Component> tooltip = new ArrayList<>();
            String talentName = talent.getName();
            int currentPoints = getCurrentPoints();
            boolean isShiftPressed = Screen.hasShiftDown();

            MutableComponent attackComponent = buildPlusValueComponent(
                    currentPoints,
                    maxPoints,
                    isShiftPressed,
                    talent::getAttackDamage
            );
