...
public class SteadyFootingTalentNode extends StatTalentButton {
    private final TalentTree talentTree;

    public SteadyFootingTalentNode(TalentTree talentTree, Talent talent, int x, int y, int maxPoints, int requiredPoints, boolean alwaysActive, OnPress onPress) {
    ...
    public List<Component> generateDynamicTooltip() {
        ...
        double statMultiplier = 3;//Change this value for each talent
        double currentRankValue = (currentPoints * statMultiplier);
        double nextRankValue = (currentRankValue + statMultiplier);
        boolean isShiftPressed = Screen.hasShiftDown();

        tooltip.add(Component.translatable("talents.steady_footing")//Change this value for each talent
        ...
        if (currentPoints == 0) {
            tooltip.add(Component.translatable("talents.player_gains")
                    .append(Component.literal(" +3.0%/6.0%/9.0% ")
                            .withStyle(Style.EMPTY.withColor(TextColor.parseColor("#00FF00"))))
                    .append(Component.translatable("talents.knockback_resistance")));//Change this value for each talent
