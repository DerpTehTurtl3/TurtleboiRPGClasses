...
public class SwiftHandsTalentNode extends StatTalentButton {
    public SwiftHandsTalentNode(TalentTree talentTree, Talent talent, int x, int y, int maxPoints, int requiredPoints, boolean alwaysActive, OnPress onPress) {
    ...
    public List<Component> generateDynamicTooltip() {
        ...
        double statMultiplier = 4;//Change this value for each talent
        double currentRankValue = (currentPoints * statMultiplier);
        double nextRankValue = (currentRankValue + statMultiplier);
        boolean isShiftPressed = Screen.hasShiftDown();

        tooltip.add(Component.translatable("talents.swift_hands")//Change this value for each talent
        ...
        if (currentPoints == 0) {
            tooltip.add(Component.translatable("talents.player_gains")
                    .append(Component.literal(" +4.0%/8.0%/12.0% ")
                            .withStyle(Style.EMPTY.withColor(TextColor.parseColor("#00FF00"))))
                    .append(Component.translatable("talents.attack_speed")));//Change this value for each talent
